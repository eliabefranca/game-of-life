...
export class Cell {
    ...
    public getNewState() {
        const aliveNeighbors = this.neighbors.filter(neighborCell => neighborCell.state === 1).length
...
export class Game {
    ...
    public start() {
        this.timeInterval = setInterval(() => this.computeNextGeneration(this), this.config.speed)
    }
}
